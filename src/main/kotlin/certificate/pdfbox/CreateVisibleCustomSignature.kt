 ...
 */
class CreateVisibleCustomSignature
/**
...
constructor(keystore: KeyStore, pin: CharArray) : CreateSignatureBase(keystore, pin) {
    ...
    private fun createVisualSignatureTemplate(srcDoc: PDDocument, pageNum: Int, rect: PDRectangle?): InputStream {
        PDDocument().use { doc ->
            ...
            form.bBox = bbox
            val font = fontPath?.let {
                PDType0Font.load(doc, FileInputStream(it))
...
//                cs.fill()
                ...
                imageFile?.also { file ->
                    ...
                    val img = PDImageXObject.createFromFileByExtension(file, doc)
                    val heightScale = imageHeight!! / img.height
                    val widthScale = width / img.width
                    val scale = heightScale.takeIf { it < widthScale } ?: widthScale
                    cs.transform(Matrix.getScaleInstance(scale, scale))

                    val imgWidthScaled = img.width * scale
                    val imgHeightScaled = img.height * scale
                    val x = (width - imgWidthScaled) / 2f / scale
                    val y = (height - imageHeight!! + (imageHeight!! - imgHeightScaled) / 2f) / scale
...
//                val fontSize = fontSize
                ...
                cs.newLineAtOffset(fontSize!!, leading)
                cs.setLeading(leading)
                line1?.also(cs::showText)
                cs.newLine()
                line2?.also(cs::showText)
                cs.endText()
            }
